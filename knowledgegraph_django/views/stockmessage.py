...
def getStockMinuteInformation(stock, minute):
    ...
    try:
        ...
        for i in range(len(data_list)):
            ans.append([convert_to_formatted_datetime(data_list[i][1]), data_list[i][6], str(
                float(data_list[i][8])/float(data_list[i][7]))])
        #### 结果集输出到csv文件 ####
...
def getStockDayInformation(stock, days):
    ...
    try:
        ...
        # result = pd.DataFrame(data_list, columns=rs.fields)
        for i in range(len(data_list)):
            ans.append([data_list[i][0], data_list[i][2],
                       data_list[i][5], data_list[i][4], data_list[i][3]])
        #### 结果集输出到csv文件 ####
        # result.to_csv("D:\\history_A_stock_k_data.csv", index=False)
        # print(ans)
        #### 登出系统 ####
